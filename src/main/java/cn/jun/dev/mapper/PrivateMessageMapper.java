...
public interface PrivateMessageMapper {
         ...
         * 发送者删除消息
         */
        @Update("UPDATE private_message SET is_deleted_by_sender = 1 WHERE id = #{id} AND sender_id = #{userId}")
        int deleteBySender(@Param("id") Long id, @Param("userId") Long userId);

        /**
         * 接收者删除消息
         */
        @Update("UPDATE private_message SET is_deleted_by_receiver = 1 WHERE id = #{id} AND receiver_id = #{userId}")
        int deleteByReceiver(@Param("id") Long id, @Param("userId") Long userId);
}
